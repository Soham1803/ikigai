...
function TaskList(props: TaskListProps) {
    ...
    return (
        <div className="flex flex-col justify-start gap-2 w-full px-4 overflow-y-auto h-[55%]">
            {props.tasks.map((task, index) => (
                <div key={index} className="flex flex-row w-full h-20 border-2 border-yellow-600 items-center justify-between p-6 rounded-xl">
                    ...
                    <Trash2 className="hover:text-red-600 active:text-red-400" onClick={() => handleRemoveTask(task.id)} />
                </div>
            ))}
        </div>
    )
